    ...
    <div id="content" class="main-content">
        <div class="container">
            <div class="container my-5 mx-auto">
                ...
                <form class="form-vertical" action="{{route('product.details.store')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group mb-4">
                        <label class="control-label">Stock:</label>
                        <input type="number" name="stock" class="form-control @error('price') is-invalid fparsley-error parsley-error @enderror" value="{{old('stock')}}">
                        @error('stock')
                    ...
                    <div class="form-group mb-4">
                        <select class="form-control  basic @error('product_id') is-invalid fparsley-error parsley-error @enderror" name="product_id">
                            ...
                            @foreach($products as $item)
                                <option value="{{$item->id}}" title="{{$item->product_names[0]->name}}">{{$item->product_names[1]->name}}</option>
                            @endforeach
                            @endisset
                        </select>
                        @error('product_id')
                    ...
                    <div class="form-group mb-4">
                        <select class="form-control  basic @error('size_id') is-invalid fparsley-error parsley-error @enderror" name="size_id">
                            ...
                            @foreach($sizes as $item)
                                <option value="{{$item->id}}">{{$item->size}} {{$item->size_unit->unit}}</option>
                            @endforeach
                            @endisset
                        </select>
                        @error('size_id')
